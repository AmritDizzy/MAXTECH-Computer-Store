		 ...
		 <section class="clearfix">
		  <ul class="new-products">
		        <?php
                    ...
                    if($row['p_discount'] > 0 )
                    {
                        $output .= "<strike>".$row['p_price']."TTD</strike>";
                        $output .= "<i>Discount: ".$row['p_discount']." %</i>";
                        $output .= "<br>";
                        $discount_amount = ($row['p_discount'] * $row['p_price'])/100;
                        $cr_amount = $row['p_price'] - $discount_amount;
                        $output .= "<span id=\"original-price\">".$cr_amount." TTD</span>";
                        if($quanity != "0")
                        {
                            $output .= "<span id=\"stock\">In Stock!</span></div>";
                        ...
                        $output .= "<br>";
                        $output .= "<span id=\"original-price\">".$row['p_price']."TTD</span>";
                        if($quanity != "0")
                        {
                            $output .= "<span id=\"stock\">In Stock!</span></div>";
                            $output .= "<div id=\"see-details\"><a href=\"product_single.php?id=".$row['p_id']."\">Buy Now</a></div>";
