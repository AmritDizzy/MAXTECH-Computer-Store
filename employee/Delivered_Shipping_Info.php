...
<section >
  ...
  <div id="main-body" >
   ...
   $query = $conn->query("SELECT * FROM sales WHERE shipment_status = 'Product Delivered' OR shipment_status = 'Product Received' order by `s_id` DESC") or die();
...
?>
        ...
        if ($row1['shipment_status'] == "Product Received") {
...
<!-- Modal Finish-->
       ...
       <td> 
        ...
        <button type="submit" name="changes" class="btn btn-primary:hover"> Product Received By Customer </button>
        </form>
...
if (isset($_POST['changes'])) {
    ...
    elseif ($_POST['status'] == 'Product Received') {
        $status = 'Product Received';
    } 
    ...
    if ($done) {
        echo "<script> document.location.href='Shipping_info.php';</script>";
    } else {
