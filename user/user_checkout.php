...
echo"<script>location.href='user_login.php';</script>";
   ...
   if(isset($_POST['submit']))
    {
        if(!empty($_POST['fname'])&& !empty($_POST['lname']) && !empty($_POST['email'])&& !empty($_POST['phone'])&& !empty($_POST['address'])&& !empty($_POST['city'])
		&& !empty($_POST['option']))
        {
          $fname = $_POST['fname'];
		  $lname = $_POST['lname'];
...
?>
   ...
   <section>
    ...
    ?>
                    ...
                    <input type="text" size="60" name="fname"  value="<?= $rowc['f_name']; ?>"  >
                    
                    <p id="cname" class="clabel">Last Name :</p>
                    <input type="text" size="60" name="lname" pattern="[A-Za-z]+" title="only letters" value="<?= $rowc['l_name']; ?>" >
